...
class _AddEditItemScreenState extends State<AddEditItemScreen> {
    ...
    try {
      ...
      if (connectedDeviceId != null && cabProvider.cabinets.isNotEmpty) {
        ...
        if (matchedCabinets.isNotEmpty) {
          ...
          debugPrint(
              '🔵 Matched cabinet by BLE device: ${matched.name} (${matched.id})');
      ...
      final rememberedCabinetId =
          await LocationMemoryService().getLastCabinetId();
      final rememberedBoxId = await LocationMemoryService().getLastBoxId();

      if (rememberedCabinetId != null) {
        final cabinetStillExists =
        ...
        if (cabinetStillExists && !_isLocationManuallyChanged) {
          ...
          if (rememberedBoxId != null) {
            final boxStillExists = cabProvider.boxes.any((b) =>
                b.id == rememberedBoxId && b.cabinetId == rememberedCabinetId);
            if (boxStillExists) {
          ...
          debugPrint(
              '✅ Restored remembered location: cabinet=$rememberedCabinetId, box=$validBoxId');
          setState(() => _isLoadingLocation = false);
          return;
        }
      }
  ...
  Future<void> _pickPhoto() async {
    ...
    if (source == ImageSource.gallery) {
      final picked =
          await ImagePicker().pickMultiImage(imageQuality: 85, maxWidth: 1080);
      ...
      final picked = await ImagePicker()
          .pickImage(source: source, imageQuality: 85, maxWidth: 1080);
          ...
          builder: (_, sc) => ListView(
            ...
            children: [
              ...
              if (fill.name.isNotEmpty)
                _suggestRow(ctx, sel, setLocal, 'name', 'Name', fill.name,
                    Icons.label_outline),
              if (fill.brand.isNotEmpty)
                _suggestRow(ctx, sel, setLocal, 'brand', 'Brand', fill.brand,
                    Icons.business_outlined),
              if (fill.description.isNotEmpty)
                _suggestRow(ctx, sel, setLocal, 'description', 'Description',
                    fill.description, Icons.description_outlined),
              if (matchedCat != null)
                _suggestRow(
                    ctx,
                    sel,
              ...
              _suggestRow(ctx, sel, setLocal, 'quantity', 'Quantity',
                  '${fill.quantity} ${fill.unit}', Icons.numbers_outlined),
              ...
              if (fill.note.isNotEmpty)
                _suggestRow(ctx, sel, setLocal, 'note', 'Note', fill.note,
                    Icons.note_outlined),
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          Icon(icon, size: 17, color: highlight ? Colors.orange : subColor),
  ...
  void _applyAutoFill(ItemAutoFill fill, Map<String, bool> sel, String? catId) {
    setState(() {
      ...
      if (sel['category'] == true && catId != null) _categoryId = catId;
  ...
  Widget build(BuildContext context) {
    ...
    final boxes =
        _cabinetId != null ? cabProvider.getBoxesForCabinet(_cabinetId!) : [];
          ...
          : Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    value: _doorId,
                    decoration: deco('Smart door (optional)',
                        hint: 'ESP32 servo and light for this item'),
                    items: const [
                      DropdownMenuItem(
                          value: null, child: Text('No smart door')),
                      DropdownMenuItem(
                          value: 'upper', child: Text('Upper door')),
                      DropdownMenuItem(
                          value: 'lower', child: Text('Lower door')),
                    ],
                    onChanged: _cabinetId == null
                        ? null
                        : (value) => setState(() => _doorId = value),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    'Take Out opens this servo and turns on its light.',
                    style: TextStyle(fontSize: 11, color: subColor),
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(Icons.calendar_today_outlined, size: 17, color: subColor),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Text(text,
                    style: TextStyle(
                        fontSize: 14,
                        color:
                            date != null ? textColor : Colors.grey.shade400)),
              ],
            ),
          ),
          if (date != null)
